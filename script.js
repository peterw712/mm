...
function parseMappings(mappingInput) {
    const mappings = {};
    const lines = mappingInput.trim().split('\n');

    lines.forEach(line => {
        const [number, note] = line.split('=').map(el => el.trim());
        mappings[number] = note;
    });

    return mappings;
}

function convertSequence(sequence, mappings) {
    return sequence.map(element => mappings[element] || element);
}
...
function generateSequence() {
    ...
    // Get the mappings from user input
    const mappingInput = document.getElementById('mapping').value;
    const mappings = parseMappings(mappingInput);

    // Convert the generated sequence using the mappings
    const convertedSequence = convertSequence(newSequence, mappings);

    document.getElementById('outputSequence').innerText = convertedSequence.join(', ');
}
